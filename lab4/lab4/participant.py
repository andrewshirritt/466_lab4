...
class Seller(Participant):
    ...
    def vote(self):
        ...
        # Seller will always commit so send commit message to Coordinator
    ...
    def run(self):
        ...
        # Receive connection from Coordinator
        ...
        # Receive global commit/abort
        msg = transaction_pb2.Transaction()
        packet = self.conn.recv(1024)
        msg.ParseFromString(packet)

        # If it is commit, commit and then ack, else abort and ack
        if msg.type == 3:
            self.commit()
...
class Buyer(Participant):
    ...
    def vote(self):
        ...
        # If it is a good price, commit, else, abort
